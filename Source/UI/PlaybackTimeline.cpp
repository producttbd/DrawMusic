...
void PlaybackTimeline::setToControlAudioSource(Rectangle<int> activeClickArea, PositionableAudioSource* audioSource)
{
...
void PlaybackTimeline::mouseDown(const juce::MouseEvent &event)
{
    if (audioSource_ != nullptr)
    {
        ...
        audioSource_->setNextReadPosition(static_cast<int64>(audioSource_->getTotalLength() * newFractionalPosition));
        repaint();
    }
}
